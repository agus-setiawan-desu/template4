        ...
        <div class="column is-one-third">
          <form @submit.prevent="onSubmit">
            <div class="field">
              ...
              <div class="control">
                  <input class="input" type="text" name="name" v-model="name" v-validate="'required'" :class="{ 'is-danger': errors.has('name') }">
            ...
            <div class="field">
              <div class="control">
                  <button class="button is-primary" :class="{ 'is-loading': busy }" :disabled="busy">{{ !category ? 'Create' : 'Update' }}</button>
                  <button style="margin-left: 10px;" type="button" class="button" @click.prevent="cancelUpdate()" v-if="category">Cancel</button>
              </div>                    
            </div>
          </form>
        </div>
          ...
          <table class="table is-striped is-fullwidth">
            ...
            <tbody>
              <tr v-for="(category, index) in categories" :key="category.key">
                  ...
                  <td><a href="#" @click.prevent="removeCategory(category)"><span class="icon has-text-danger"><i class="fa fa-lg fa-times-circle"></i></span></a></td>
              </tr>              
            </tbody>
          </table>
        </div>                
    ...
    methods: {
      ...
      removeCategory (category) {
        this.$swal({
          ...
          if (ok) {
            this.$store.dispatch('product/removeCategory', { category: category })
          }
        })
      },
      cancelUpdate () {
